...
// Ingredients grouped by their editor section (.grp). Used in
// the cook-mode sidebar AND the mobile ingredients drawer (the
// sidebar is display: none on phones), plus MealCookMode's
// per-recipe lists. `color` tints the qty column — the meal
// cook mode passes each recipe's legend colour.
export function GroupedIngredientList({ ingredients, color = "var(--accent)" }) {
  const byGrp = {};
  const order = [];
      ...
      <ul style={{ listStyle: "none", padding: 0, margin: 0, fontSize: 12.5 }}>
        {byGrp[g].map((i, idx) => (
          <li key={idx} style={{ display: "grid", gridTemplateColumns: "60px 1fr", gap: 8, padding: "4px 0", borderBottom: "1px dotted var(--rule)" }}>
            <span className="mono" style={{ color }}>{formatIngredientQty(i)}</span>
...
export function CookMode({ recipe, steps, ingredients, finishTime, setFinishTime, onClose, authEmail, onSaveRecipe }) {
  ...
  // Mobile-only ingredients drawer — the sidebar that normally
  // holds the list is display: none on phones.
    ...
    <div className="cookmode-overlay" data-screen-label={`03 Cook mode: ${recipe.title}`}>
      ...
      <div className="cookmode-body">
        ...
        <div className="cookmode-side">
          ...
          <GroupedIngredientList ingredients={ingredients} />
        </div>
      ...
      <Drawer open={ingsOpen} onClose={() => setIngsOpen(false)} title={t("ingredientsOnHand")}>
        <GroupedIngredientList ingredients={ingredients} />
      </Drawer>
    </div>
  );
