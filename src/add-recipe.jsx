...
export function AddRecipe({ onClose, onSave, onDelete, authEmail, initialRecipe = null, usedCuisines = [], usedAuthors = [] }) {
  ...
  // Ask the worker to generate a hero image from the recipe's
  // title + ingredients + early steps in the family-cookbook style
  // (rustic wood, warm light, neutral ceramics — full prompt
  // template lives in worker/index.js). The ingredients tell the
  // model what physically goes into the dish; the first few steps
  // pin down the form factor and cookware (rolled, layered,
  // skillet, glazed) that the title alone usually misses. The
  // image is generated by gpt-image-1, persisted to R2, and the
  // URL replaces draft.photo just like a regular upload.
  const generatePhoto = async () => {
    if (!draft.title?.trim()) {
      setPhotoError(t("aiGenerateNeedsTitle"));
      return;
    }
    setPhotoError(null);
    setGeneratingPhoto(true);
    try {
      const res = await fetch("/api/admin/ai/hero-image", {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json", Accept: "application/json" },
        body: JSON.stringify({
      ...
      if (!res.ok) {
        const { error } = await res.json().catch(() => ({}));
        throw new Error(error || `Image generation failed (${res.status})`);
      }
      const { url } = await res.json();
      setDraft(d => ({ ...d, photo: url }));
    } catch (err) {
      setPhotoError(err.message || "Image generation failed");
    } finally {
      setGeneratingPhoto(false);
    }
  };

  // Roughly estimate per-serving nutrition from the current
  // ingredient list. Useful when AI extraction returned zeros or
  // when the cook is editing a recipe that never had values.
  const estimateNutrition = async () => {
    if (!draft.ingredients?.length) {
      setNutritionError(t("nutritionNeedsIngredients"));
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        const { error } = await res.json().catch(() => ({}));
        throw new Error(error || `Estimate failed (${res.status})`);
      }
      const nutrition = await res.json();
      setDraft(d => ({ ...d, nutrition }));
    } catch (err) {
      setNutritionError(err.message || "Estimate failed");
    } finally {
