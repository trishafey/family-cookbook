...
export function AddRecipe({ onClose, onSave, onDelete, authEmail, initialRecipe = null, usedCuisines = [], usedAuthors = [] }) {
  ...
  // title in the family-cookbook style (rustic wood, warm light,
  // neutral ceramics — full prompt template lives in worker/index.js).
  // The image is generated by gpt-image-1, persisted to R2, and the
  ...
  const generatePhoto = async () => {
    ...
    try {
      const res = await fetch("/api/admin/ai/hero-image", {
        ...
        credentials: "include",
        headers: { "Content-Type": "application/json", Accept: "application/json" },
        body: JSON.stringify({ title: draft.title, course: draft.course }),
      });
      if (!res.ok) {
        const { error } = await res.json().catch(() => ({}));
